...
// single Point light source using OpenGL 3.3 or a forward compatible
...
// Global Variables
const char* APP_TITLE = "Introduction to Modern OpenGL - Point Light";
...
int main()
{
	...
	glm::vec3 modelPos[] = {
		...
		glm::vec3(-2.0f, 0.0f, 2.0f)	// lamp post
	};
	...
	while (!glfwWindowShouldClose(gWindow))
	{
		...
		// Move the lamp and light
		angle += (float)deltaTime * 50.0f;
		modelPos[6].x = 3.0f * sinf(glm::radians(angle));
		modelPos[6].z = 14.0f + 10.0f * cosf(glm::radians(angle));
		lightPos = modelPos[6];
		lightPos.y += 3.8f; // offset to light bulb found by trial and error
		...
		lightingShader.setUniform("pointLight.ambient",  glm::vec3(0.2f, 0.2f, 0.2f));
		lightingShader.setUniform("pointLight.diffuse",  glm::vec3(1.0f, 1.0f, 1.0f));
		lightingShader.setUniform("pointLight.specular", glm::vec3(1.0f, 1.0f, 1.0f));
		lightingShader.setUniform("pointLight.position", lightPos);
